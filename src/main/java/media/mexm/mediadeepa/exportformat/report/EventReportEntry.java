...
public record EventReportEntry(LavfiMtdEvent event, Optional<Duration> sourceDuration) implements ReportEntry {
	...
	public DomContent toDomContent() {
		...
		final var spanStart = span(attrs(".start.value"), durationToString(event.start()));
		...
		if (end != null && end.toMillis() > 0 || sourceDuration.isPresent()) {
			...
			spanEnd = span(attrs(".end.value"), durationToString(end));
		} else {
			end = null;
		}
		...
		if (end != null) {
			spanDuration = span(attrs(".duration.value"), durationToString(end.minus(event.start())));
		}
