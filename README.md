...
Build jar will be founded on `target` directory as `mediadeepa-0.0.20.jar`

And simply run the application with

```bash
java -jar mediadeepa-0.0.20.jar
```

Mediadeepa contain embedded help, displayed with the `-h` parameter.

You can set the command line parameters with `java -jar mediadeepa-0.0.20.jar [parameters]`.

> Example: `java -jar mediadeepa-0.0.20.jar -h`
