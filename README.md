...
sudo dpkg -i mediadeepa-0.0.44.deb

# RPM file on RHEL/CentOS Linux distribs
sudo rpm -U mediadeepa-0.0.44.rpm
```
...
On Windows/macOS, just run `java -jar mediadeepa-0.0.44.jar [options]`.

And simply run the application with `java -jar mediadeepa-0.0.44.jar`.

Mediadeepa contain embedded help, displayed with the `-h` parameter.

You can set the command line parameters with `java -jar mediadeepa-0.0.44.jar [parameters]`.
...
Build jar will be founded on `target` directory as `mediadeepa-0.0.44.jar`
